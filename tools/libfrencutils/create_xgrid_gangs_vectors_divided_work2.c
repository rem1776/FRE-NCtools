...
#define MAX_V 8
  ...
  int num_gangs, ngang, delta_gang, num_vectors;
  ...
  xgrid_area_vector = (double *)malloc(nx2*ny2*sizeof(double));
  xgrid_clon_vector = (double *)malloc(nx2*ny2*sizeof(double));
  xgrid_clat_vector = (double *)malloc(nx2*ny2*sizeof(double));
  ij2_vector = (int *)malloc(nx2*ny2*sizeof(int));
...
#pragma acc data copyin(lon_out[0:(nx2+1)*(ny2+1)], lat_out[0:(nx2+1)*(ny2+1)], mask_in[0:nx1*ny1], \
			...
			delta_gang, ngang, num_gangs, num_vectors)			\
  ...
  create( xgrid_area_vector[0:nx2*ny2], xgrid_clon_vector[0:nx2*ny2], xgrid_clat_vector [0:nx2*ny2], \
	  ij2_vector[0:nx2*ny2])				\
  copy(nxgrid)
{
...
#pragma acc parallel num_gangs(num_gangs) vector_length(num_vectors)	\
  private(ij2_vector[0:nx2*ny2],xgrid_area_vector[0:nx2*ny2], xgrid_clon_vector[0:nx2*ny2],xgrid_clat_vector[0:nx2*ny2])
...
#pragma acc loop seq 
	  ...
	  */
	  ...
	  if (  (n_out = clip_2dx2d( x1_in, y1_in, n1_in, x2_in, y2_in, n2_in, x_out, y_out )) > 0) {
	    ...
	    min_area = min(area_in[j1*nx1+i1], area_out[j2*nx2+i2]);
	    if( xarea/min_area > AREA_RATIO_THRESH ) {
	      xgrid_area_vector[ij2] = xarea;
	      xgrid_clon_vector[ij2] = poly_ctrlon(x_out, y_out, n_out, lon_in_avg);
	      xgrid_clat_vector[ij2] = poly_ctrlat (x_out, y_out, n_out );
	      ij2_vector[ij2] = ij2;
	      nxgrid++;
	    }//if
	  }//if
