...
#define MAX_V 8
  ...
  int num_gangs, ngang, delta_gang, num_vectors, nvectors, delta_vector;
  double *xgrid_area_gang, *xgrid_clon_gang, *xgrid_clat_gang;
  double *xgrid_area_vector, *xgrid_clon_vector, *xgrid_clat_vector;
  int *i_in_gang, *j_in_gang, *i_out_gang, *j_out_gang;
  int *ij2_vector;
  ...
  num_vectors=128;
  delta_vector=nx2*ny2/num_vectors;
  nvectors=delta_vector;

  //allocate
  xgrid_area_gang = (double *)malloc(num_gangs*ngang*sizeof(double));
  xgrid_clon_gang = (double *)malloc(num_gangs*ngang*sizeof(double));
  xgrid_clat_gang = (double *)malloc(num_gangs*ngang*sizeof(double));
  i_in_gang = (int *)malloc(num_gangs*ngang*sizeof(int));
  j_in_gang = (int *)malloc(num_gangs*ngang*sizeof(int));
  i_out_gang = (int *)malloc(num_gangs*ngang*sizeof(int));
  j_out_gang = (int *)malloc(num_gangs*ngang*sizeof(int));

  xgrid_area_vector = (double *)malloc(num_vectors*nvectors*sizeof(double));
  xgrid_clon_vector = (double *)malloc(num_vectors*nvectors*sizeof(double));
  xgrid_clat_vector = (double *)malloc(num_vectors*nvectors*sizeof(double));
  ij2_vector = (int *)malloc(num_vectors*nvectors*sizeof(int));

#pragma acc data copyin(lon_out[0:(nx2+1)*(ny2+1)], lat_out[0:(nx2+1)*(ny2+1)], mask_in[0:nx1*ny1], \
			area_in[0:nx1*ny1], area_out[0:nx2*ny2],	\
			lon_in[0:(nx1+1)*(ny1+1)], lat_in[0:(nx1+1)*(ny1+1)], \
			nx1, ny1, nx2, ny2, nx1p, nx2p, \
			delta_gang, ngang, num_gangs, delta_vector, num_vectors, nvectors) \
  copyout( xgrid_area_gang[0:num_gangs*ngang], xgrid_clon_gang[0:num_gangs*ngang], \
	   xgrid_clat_gang[0:num_gangs*ngang],				\
	   j_in_gang[0:num_gangs*ngang], i_out_gang[0:num_gangs*ngang],	\
	   j_out_gang[0:num_gangs*ngang],i_in_gang[0:num_gangs*ngang])	\
  create( xgrid_area_vector[0:nvectors*num_vectors], xgrid_clon_vector[0:nvectors*num_vectors], xgrid_clat_vector [0:nvectors*num_vectors], \
	  ij2_vector[0:nvectors*num_vectors])				\
  copy(nxgrid)
{

#pragma acc parallel loop independent
  for( int i=0 ; i<num_gangs*ngang ; i++) {
    i_in_gang[i]=-99;
  }
  
#pragma acc parallel num_gangs(num_gangs) vector_length(num_vectors)	\
  private(ij2_vector[0:nvectors*num_vectors],xgrid_area_vector[0:nvectors*num_vectors], xgrid_clon_vector[0:nvectors*num_vectors],xgrid_clat_vector[0:nvectors*num_vectors])
#pragma acc loop independent gang reduction(+:nxgrid) 
  for (int igang=0; igang<num_gangs ; igang++) {
    int start_here_gang, end_here_gang;
    int tcount=0;
    
    start_here_gang = igang * delta_gang;
    end_here_gang = start_here_gang + delta_gang;
    if(igang==(num_gangs-1) ) end_here_gang = nx1*ny1;
#pragma acc loop seq 
    for(ij1=start_here_gang; ij1<end_here_gang ; ij1++ ) {       
...
#pragma acc loop vector independent reduction(+:nxgrid)
	for( int ivector=0 ; ivector<num_vectors ; ivector++){
	  int start_here_vector, end_here_vector;
	  int vector_count;

	  vector_count=0;
	  start_here_vector=ivector*delta_vector;
	  end_here_vector=start_here_vector+delta_vector;
#pragma acc loop seq 
	  for( int ij2=0 ; ij2<nvectors*num_vectors ; ij2++){
	    ij2_vector[ij2]=-99;
	  }

#pragma acc loop seq 
	  for( int ij2=start_here_vector; ij2<end_here_vector; ij2++) {
...
#pragma acc loop seq 
	    ...
	    */
	    ...
	    if (  (n_out = clip_2dx2d( x1_in, y1_in, n1_in, x2_in, y2_in, n2_in, x_out, y_out )) > 0) {
	      ...
	      min_area = min(area_in[j1*nx1+i1], area_out[j2*nx2+i2]);
	      if( xarea/min_area > AREA_RATIO_THRESH ) {
		xgrid_area_vector[ivector*nvectors+vector_count] = xarea;
		xgrid_clon_vector[ivector*nvectors+vector_count] = poly_ctrlon(x_out, y_out, n_out, lon_in_avg);
		xgrid_clat_vector[ivector*nvectors+vector_count] = poly_ctrlat (x_out, y_out, n_out );
		ij2_vector[ivector*nvectors+vector_count] = ij2;
		nxgrid++;
		vector_count++;
		//if(igang==0) printf("%d %d %d %d %d\n", igang, ivector, vector_count, ivector*nvectors+vector_count, ij2);
	      }//if
	    }//if
	  }//ij2
 	}//ivector
#pragma acc loop seq
	int icount=0;
	for(int ij2=0; ij2<nvectors*num_vectors; ij2++) {
	  if( ij2_vector[ij2] != -99 ) {
	    xgrid_area_gang[igang*ngang+tcount+icount]=xgrid_area_vector[ij2];
	    xgrid_clon_gang[igang*ngang+tcount+icount]=xgrid_clon_vector[ij2];
	    xgrid_clat_gang[igang*ngang+tcount+icount]=xgrid_clat_vector[ij2];
	    i_in_gang[igang*ngang+tcount+icount]=i1;
	    j_in_gang[igang*ngang+tcount+icount]=j1;
	    i_out_gang[igang*ngang+tcount+icount]=ij2%nx2;
	    j_out_gang[igang*ngang+tcount+icount]=ij2/nx2;
	    icount++;
	    //if(igang==0) printf("**%d %d %d %d\n", igang, icount, ij2, ij2_vector[ij2]);
	  }
	} 
	//printf("%d %d\n",igang, icount);
 ...
 int icount=0;
 for(int j=0; j<ngang*num_gangs; j++ ) {
   if( i_in_gang[j] != -99 ){
     xgrid_area[icount]=xgrid_area_gang[j];
     xgrid_clon[icount]=xgrid_clon_gang[j];
     xgrid_clat[icount]=xgrid_clat_gang[j];
     i_in[icount]=i_in_gang[j];
     j_in[icount]=j_in_gang[j];
     i_out[icount]=i_out_gang[j];
     j_out[icount]=j_out_gang[j];
     icount++;
   }
 }
